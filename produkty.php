		...
		$sql = "SELECT * FROM producenci";
			...
			$result=mysqli_query($polaczenie,$sql);//wynik
				...
				do
				{
				  ...
				  echo"<option >".$dane['Nazwa_Producenta']."</option>";	
...
<p style="margin-left:15px;"><input type="submit" value="Wyślij" /></p style>
  ...
  <?php
		...
		if ($polaczenie!=NULL)
		{
			...
			if (isset($sortowanie) && isset($typ)&& isset($producent))
			{
				switch($sortowanie)
				{
					case "malejąco":
				...
				$sql = "SELECT DISTINCT * FROM produkty 
				LEFT JOIN zdjecia ON produkty.ID_Produktu=zdjecia.ID_Produktu 
				LEFT JOIN producenci ON produkty.ID_Producenta=producenci.ID_Producenta 
				WHERE produkty.Typ ='".$typ."' AND producenci.Nazwa_Producenta ='".$producent."' GROUP BY produkty.ID_Produktu ORDER BY produkty.Cena ".$s;
			}
